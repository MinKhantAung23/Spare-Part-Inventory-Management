...
export default function QuickSearchPage() {
  ...
  // Real-time asynchronous data fetching for the detail layout block
  const { data: partDetails, isLoading: loadingDetails } = useSparePartsById(selectedPartId || "");
      ...
      <div className="lg:col-span-8 bg-white border border-slate-200 rounded-3xl p-8 overflow-y-auto custom-scrollbar shadow-sm flex flex-col justify-stretch">
        {loadingDetails ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 border-2 border-dashed border-slate-100 rounded-2xl">
            ...
            <p className="text-xs text-slate-400 mt-1">Drill down into the brand structure map to preview full spec data assets.</p>
          </div>
        )}
      </div>
    </div>
