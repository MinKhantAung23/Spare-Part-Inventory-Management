...
import { useStockInQuery } from "@/hooks/useStockIn";
import { useStockInStore } from "@/store/use-stock-in-store";
import StockInTable from "@/components/stock/StockInTable";
import StockInFilterDialog from "@/components/stock/StockInFilterDialog";
...
export default function StockInPage() {
  ...
  const { data: response, isLoading } = useStockInQuery({
    page,
    search: searchQuery,
  ...
  return (
    <div className="space-y-6 font-padauk">
      ...
      <div className="flex justify-between items-center flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-slate-800">ပစ္စည်းအဝင်စာရင်း</h1>
        <Link
          href="/stock-in/receive-stock/"
          className="flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-white transition-all hover:bg-blue-600"
        >
          <Plus size={16} />
          စတော့ထည့်မည်
        </Link>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by spare part name, brand, model..."
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-white border border-dashed border-slate-200 rounded-3xl text-slate-400 gap-3">
          ...
          <p className="text-sm font-semibold text-slate-500">No stock records found</p>
        ...
        <StockInTable data={visibleItems} />
      )}
