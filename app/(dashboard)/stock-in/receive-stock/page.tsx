...
export default function StockInFormPage() {
  ...
  return (
    <div className="w-full mx-auto p-4 md:p-8 font-padauk h-fit bg-background">
      <Link
        href="/stock-in"
        className="flex items-center text-slate-500 hover:text-primary mb-6 transition-colors text-sm"
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm italic">
                ပစ္စည်းသွင်းခြင်း အချက်အလက်များ
              </p>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="pt-8 mb-20">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <StepBlock step={1} currentStep={step} label="Brand">
              <SearchableSelect
                options={cascade.brands.map((b: any) => ({
                  id: b.id,
                  name: b.name,
                }))}
            ...
            <StepBlock step={2} currentStep={step} label="Model">
              <SearchableSelect
                options={cascade.models.map((m: any) => ({
                  id: m.id,
                  name: m.name,
                }))}
            ...
            {/* Step 3 — Category */}
            <StepBlock step={3} currentStep={step} label="အပိုပစ္စည်းအမျိုးအစား">
              <SearchableSelect
                options={cascade.categories.map((c: any) => ({
                  id: c.id,
                  name: c.name,
                }))}
            ...
            {/* Step 4 — Spare Part */}
            <StepBlock step={4} currentStep={step} label="အပိုပစ္စည်း">
              <SearchableSelect
                ...
                onChange={(id: any) =>
                  cascade.selectPart(id ? Number(id) : null)
                }
...
function StepBlock({
  ...
  return (
    <div
      className={`space-y-1.5 transition-opacity ${isActive ? "opacity-100" : "opacity-40 pointer-events-none"}`}
    >
